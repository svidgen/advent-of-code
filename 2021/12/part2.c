...
int has_visited(dict(int) * v, String * room) {
	return v->has(v, room->items) && v->get(v, room->items) > 0;
}

void visit(dict(int) * v, String * room) {
	int visits = v->has(v, room->items) ? v->get(v, room->items) : 0;
	visits++;
	v->set(v, room->items, visits);
}

void unvisit(dict(int) * v, String * room) {
	int visits = v->has(v, room->items) ? v->get(v, room->items) : 1;
	visits--;
	v->set(v, room->items, visits);
}

array(Path) * find_paths(map(String) * m, dict(int) * visited, String * start, String * end, int allow_revisit) {
	array(Path) * paths = new_array(Path);

	int allow_child_revisit = allow_revisit;

	if (has_visited(visited, start)) {
		if (allow_revisit && !String_eq(start, new_string("start")) && !String_eq(start, new_string("end"))) {
			allow_child_revisit = 0;
		} else {
			return paths;
		}
	}
	...
	if (String_eq(start, end)) {
		...
		for (int i = 0; i < children->length; i++) {
			array(Path) * child_paths = find_paths(m, visited, &(children->items[i]), end, allow_child_revisit);
...
int main() {
	...
	dict(int) * visited = new_dict(int);
	array(Path) * paths = find_paths(m, visited, new_string("start"), new_string("end"), 1);
